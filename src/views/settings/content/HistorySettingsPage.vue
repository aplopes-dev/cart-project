      ...
      <div class="max-w-[1408px] mx-auto">
        ...
        <div class="bg-[#FAFAFA] p-8 mb-8">
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div>
              <label class="block font-archivo text-sm mb-2">{{ $t('content.history.form.text') }}</label>
              <textarea 
...
const toggleVisibility = (index) => {
  historyItems.value[index].visible = !historyItems.value[index].visible
}

const editItem = (index) => {
  formData.value = { ...historyItems.value[index] }
  historyItems.value.splice(index, 1)
}

const deleteItem = (index) => {
  if (confirm(t('content.history.confirmDelete'))) {
    historyItems.value.splice(index, 1)
  }
}
</script>
